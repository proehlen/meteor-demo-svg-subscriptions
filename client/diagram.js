...
function getContainer(){
  if (!container){
   container = d3.select("#diagram").append("svg")
        .attr("width", 1000)
        .attr("height", 75);
  }
  return container;
...
function renderSvg() {
    const svgContainer = getContainer();
    const teeth = Teeth.find();

    // Create svg teeth if they don't exist yet
    svgContainer
      .selectAll(".teeth")
      .data(teeth.fetch())
      .enter()
      ...
      .attr("d", TOOTH)
      .attr("transform", (data) => {
          return `translate(${data._id *100},10),scale(.1)`;
      })
      .attr("class", "teeth");

    // Dynamically update color based on current meteor data
    svgContainer
      .selectAll(".teeth")
      .data(teeth.fetch())
